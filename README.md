...
| ID | 学生架构 | 说明 |
|----|----------|------|
| S0 | `yolo_unet` | YOLO11m backbone/neck + U-Net 解码头，**from scratch**（从 `yolo11m-seg.yaml` 随机初始化，**backbone/neck 全解冻、端到端训练，22.35M 可训练**） |
...
- **初始化口径统一为 from scratch**：所有实验（含 S0 及蒸馏学生 S1~S3_attn）均为随机初始化；
  S0 的 scratch 由 `--student-scratch` 控制（编排器对 `yolo_unet` 实验默认开启）。
- **注意（重跑）**：`YoloUNetSemanticStudent` 的 YOLO11m backbone/neck 现已**全解冻**且**从 scratch**。
  因此 S0 与学生蒸馏实验（S1/S1_attn/S2/S2_attn/S3/S3_attn）需**重跑**，旧结果（预训练初始化 + 仅训
  3.71M 解码器）作废。
